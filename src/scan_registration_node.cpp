...
scanRegistrationNode::scanRegistrationNode(scanRegistrationOption option):
    scan_registration_handler_(std::make_unique<scanRegistration>(std::move(option))) {
    sub_laser_cloud_ = nh_.subscribe(
            "velodyne_points", 100,
            &scanRegistrationNode::laserCloudCallback, this);
...
void scanRegistrationNode::laserCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& laser_cloud_msg) {
...
void scanRegistrationNode::pclToMsg(const pcl::PointCloud<PointType>::Ptr laser_cloud, sensor_msgs::PointCloud2 &laser_cloud_out_msg, const ros::Time timestamp) {
